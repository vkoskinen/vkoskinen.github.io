...
// Module
___CSS_LOADER_EXPORT___.push([module.id, ".leaflet-pane.leaflet-lhl-raised-pane {\n\tz-index: 620;\n}\n\n.leaflet-pane.leaflet-lhl-almost-over-pane {\n\tz-index: 201;\n}", "",{"version":3,"sources":["webpack://./src/panes.css"],"names":[],"mappings":"AAAA;CACC,YAAY;AACb;;AAEA;CACC,YAAY;AACb","sourcesContent":[".leaflet-pane.leaflet-lhl-raised-pane {\n\tz-index: 620;\n}\n\n.leaflet-pane.leaflet-lhl-almost-over-pane {\n\tz-index: 201;\n}"],"sourceRoot":""}]);
// Exports
...
function createHighlightableLayerClass(BaseClass, cloneMethods, defaultOptions) {
    ...
    var result = /** @class */ (function (_super) {
        ...
        function HighlightableLayer() {
            ...
            _this.realOptions = Object.assign(Object.create(Object.getPrototypeOf(_this.options)), __assign(__assign({ generateStyles: styles_1.generatePolygonStyles }, defaultOptions), _this.options));
            _this.layers = {};
            for (var _b = 0, _c = Object.keys((_a = _this.realOptions.generateStyles(_this.realOptions, new leaflet_1.SVG())) !== null && _a !== void 0 ? _a : {}); _b < _c.length; _b++) {
                var layerName = _c[_b];
                if (layerName !== "main") {
                    _this.layers[layerName] = new (BaseClass.bind.apply(BaseClass, __spreadArrays([void 0], args)))();
                    _this.layers[layerName].options.interactive = false;
                    // Workaround to avoid error when calling setStyle() on Polyline without points
                    if (BaseClass === leaflet_1.Polyline || BaseClass.prototype instanceof leaflet_1.Polyline) {
                        _this.layers[layerName]._updateBounds = function () {
        ...
        HighlightableLayer.prototype.beforeAdd = function (map) {
        ...
        HighlightableLayer.prototype.setStyle = function (style) {
            ...
            var renderer = this._renderer || new leaflet_1.SVG();
            ...
            var styles = (_c = (_b = (_a = this.realOptions).generateStyles) === null || _b === void 0 ? void 0 : _b.call(_a, Object.assign(utils_1.clone(this.realOptions), { opacity: 1 }), renderer)) !== null && _c !== void 0 ? _c : { main: utils_1.clone(this.realOptions) };
            if (styles.main.pane)
                panes_1.setLayerPane(this, styles.main.pane);
            if (styles.main.renderer)
                panes_1.setLayerRenderer(this, styles.main.renderer);
            _super.prototype.setStyle.call(this, styles.main);
            for (var _i = 0, _e = Object.keys(this.layers); _i < _e.length; _i++) {
                var layerName = _e[_i];
                if (styles[layerName].pane)
                    panes_1.setLayerPane(this.layers[layerName], styles[layerName].pane);
                if (styles[layerName].renderer)
                    panes_1.setLayerRenderer(this.layers[layerName], styles[layerName].renderer);
                this.layers[layerName].setStyle(styles[layerName]);
            }
            if (renderer._container)
                renderer._container.style.opacity = "" + ((_d = this.realOptions.opacity) !== null && _d !== void 0 ? _d : 1);
            return this;
        };
        HighlightableLayer.prototype._updateBounds = function () {
            // Workaround to avoid error when calling setStyle() on Polyline without points
            if (!(this instanceof leaflet_1.Polyline) || this._rawPxBounds)
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.setLayerRenderer = exports.setLayerPane = void 0;
var leaflet_1 = __webpack_require__(453);
...
function setLayerRenderer(layer, renderer) {
    ...
    if (layer._renderer) {
        ...
        layer._renderer._layers[leaflet_1.Util.stamp(layer)] = layer;
        layer._renderer._updateStyle(layer);
        layer._renderer._addPath(layer);
    }
}
exports.setLayerRenderer = setLayerRenderer;
...
function generatePolygonStyles(options, renderer) {
    ...
    var lineWeight = (options.outlineColor == null && options.outlineWeight == null && isBright) ? Math.round(options.weight / 1.6) : options.weight;
    var outlineWeight = (_b = options.outlineWeight) !== null && _b !== void 0 ? _b : (lineWeight * 2);
    return {
        main: __assign(__assign({}, options), { renderer: renderer, weight: outlineWeight, opacity: 0 }),
        fill: __assign(__assign({}, options), { renderer: renderer, stroke: false }),
        outline: __assign(__assign({}, options), { color: outlineColor, weight: outlineWeight, renderer: renderer, fill: false }),
        border: __assign(__assign({}, options), { weight: lineWeight, renderer: renderer, fill: false })
    };
}
...
function generatePolylineStyles(options, renderer) {
    ...
    var outlineWeight = (_b = options.outlineWeight) !== null && _b !== void 0 ? _b : (lineWeight * 2);
    return {
        outline: __assign(__assign({}, options), { color: outlineColor, weight: outlineWeight, renderer: renderer }),
        line: __assign(__assign({}, options), { weight: lineWeight, renderer: renderer }),
        main: {
            opacity: 0,
